...
function filaAceptable(
  ...
  if (!findesMesCuadra(nf)) return false
  if (nf > OBJETIVO_FINDES_MES && nf > nfAntes) return false
  if (
...
/** Cubre mínimos sin dejar findes fuera de 2–3 ni abrir un finde partido. */
...
export function cruzarFindesEntreAgentes(
  ...
  resolverFindesGrupo(cuadrante, ids, turno, anio, mes, pisos)
}

function pisosDeTurno(
  turno: TurnoOperativoMes,
  anio: number,
  mes: number,
  nDias: number,
  ...
  if (!opciones?.minimosSemana || !opciones.puestos?.length) return undefined
  const pisos: number[] = []
  for (let dia = 1; dia <= nDias; dia++) {
    const minimos = minimosParaFecha(
      isoFechaCuadrante(anio, mes, dia),
      eventos,
      opciones.minimosSemana,
      opciones.puestos,
    )
    pisos.push(totalMinimosTurno(minimos, turno, opciones.puestos))
  }
  return pisos
}
...
function cruzarFindesEnCuadrante(
  ...
  const grupos = agentesPorTurnoMes(cuadrante, agenteIds, planAnual, mes, nDias)
  for (const [turno, ids] of grupos) {
    cruzarFindesEntreAgentes(
      cuadrante,
      ids,
      turno,
      anio,
      mes,
      pisosDeTurno(turno, anio, mes, nDias, eventos, opciones),
    )
  }
}
...
function construirCuadranteInicial(
  ...
  const nDias = diasDelMes(anio, mes)
  const cobertura: Record<'M' | 'T' | 'N', number[]> = {
    M: Array.from({ length: nDias }, () => 0),
    T: Array.from({ length: nDias }, () => 0),
    N: Array.from({ length: nDias }, () => 0),
  }
  const usoFinde: Record<'M' | 'T' | 'N', Map<string, number>> = {
    M: new Map(),
    T: new Map(),
    N: new Map(),
  }
  ...
  for (const id of agenteIds) {
    ...
    if (turnoBase === 'M' || turnoBase === 'T' || turnoBase === 'N') {
      const patrones = patronesAtomicos(turnoBase, anio, mes)
      if (patrones.length > 0) {
        const fila = elegirPatronAtomico(
          patrones,
          cobertura[turnoBase],
          usoFinde[turnoBase],
          anio,

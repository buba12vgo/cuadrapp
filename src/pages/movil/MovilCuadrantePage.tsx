  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="px-1 text-sm text-slate-500">
        Cuadrante del mes. Solo jefes de servicio y responsables. No se puede editar.
      </p>
      {datos.loading ? (
        <p className="px-1 text-sm text-slate-500">Cargando cuadrante…</p>
      ) : null}
      {datos.error ? (
      ...
      {Array.from({ length: datos.nDias }, (_, indice) => {
        ...
        const fuera = datos.jefes.flatMap((agente) => {
          const turno = (datos.cuadrante[agente.id]?.[indice] ?? 'D') as Turno
          if (turno === 'M' || turno === 'T' || turno === 'N' || turno === 'MT') {
            return []
          }
          const etiqueta = turno === 'P' || turno === 'L' ? 'Permiso' : turno === 'V' ? 'Vacaciones' : 'Descanso'
          return [`${agente.nombre} (${etiqueta})`]
        })
        ...
        return (
          ...
          >
              ...
              <div>
                <p
                  className={`text-[11px] font-bold uppercase tracking-wide ${
                    especial ? 'text-rose-600' : 'text-slate-400'
                  }`}
                >
                  {corto} · {largo}
                </p>
                <p className={`text-3xl font-extrabold leading-none ${especial ? 'text-rose-700' : 'text-slate-950'}`}>
                  {dia}
                </p>
              </div>
            ...
            <div className="flex flex-col gap-2 px-3 pb-3">
              {franjas.length === 0 ? (
                <p className="px-1 text-sm text-slate-500">Nadie de servicio.</p>
              ) : (
                franjas.map((franja) => (
